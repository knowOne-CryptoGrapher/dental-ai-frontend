...
- Deploy the self-serve signup lockdown (`SELF_SERVE_TIERS_ENABLED`, committed as `a4f7589a`, see Done ✅) and the Task B2 "Coming Soon" UI on top of it (committed as `b0a7d23a`, see Done ✅) — both batch together, deploy in one shot. **Task B (B1 + B2) is now fully done**, nothing further pending on this specific item.
