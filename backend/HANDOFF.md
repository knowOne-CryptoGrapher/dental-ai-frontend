...
**Backend revision:** dental-ai-backend-00078-9j6 (deployed 2026-08-07 — all of today's batched work is now live in production, see "Deployed to production" entry below)
...
- **26 tests need updating to send a real `X-Retell-Signature`** now that `retell_api_router.py`'s auth fix is written (see Urgent above, not yet deployed): `test_multi_tenant_isolation.py` (5 tests) and `test_retell_hard_suite.py` (21 tests) currently call these endpoints with no signature at all and now correctly get `401`. Pattern to use: `retell.lib.webhook_auth.symmetric["sign"](body_str, RETELL_API_KEY)`, confirmed working during this fix's local verification.
- **`backend/routers/retell_api_router.py`'s auth fix is written and locally verified but not yet committed to git** — holding per this task's scope (only `HANDOFF.md` was asked to be committed); code changes sit uncommitted in the working tree pending the coordinated live-Retell verification and an explicit commit instruction.
...
### Urgent ⚠️
- **Full production security/leakage/load audit run 2026-08-07 against the live deploy — 2 critical findings, not yet fixed (audit-only, per instruction).** Full findings reported in-conversation, not duplicated in full here:
  1. **`retell_api_router.py`'s 7 patient/appointment/provider endpoints (lookup-patient, book-appointment, cancel-appointment, register-patient, etc.) have no real authentication in production** — they read an `x_retell_signature` header and API key but discard both (`_ = (retell_api_key, x_retell_signature, body_bytes)`); a working `verify_retell_signature()` HMAC function exists in the same file but is never called. Confirmed live: `list-providers` and `lookup-patient` both return real `200` data for a guessed `practice_id` with zero credentials. Anyone with a `practice_id` (a UUID, not a secret) can read/write patient and appointment data for any practice.
     - **FIX WRITTEN AND VERIFIED LOCALLY, NOT YET DEPLOYED.** Root-caused first (didn't just wire up the existing local function — verified against Retell's own docs and the `retell-sdk` package's actual source (`retell/lib/webhook_auth.py`) that the local `verify_retell_signature()` used the wrong header format (`t=`,`v1=` — the Stripe convention) and would never have matched a real Retell signature (`v=`,`d=`) even if called. Removed it entirely. Instead reused `utils/retell_security.py`'s `verify_retell_webhook_signature()` — already correct, already used by `/api/retell/webhook` — confirmed byte-for-byte identical to the SDK's own algorithm, called with `RETELL_API_KEY` (confirmed via Retell's docs to be the correct secret for function-call signing, distinct from `RETELL_WEBHOOK_SECRET` used for webhook events) instead of the webhook secret. All 7 endpoints now call a new shared `_verify_retell_call()` helper as their first line; the `x_retell_signature: str = Header(None)` params and dead imports (`Header`, `hmac`, `hashlib`) were removed.
     - Local test, both directions confirmed: no signature → `401 "Missing webhook signature header"`; stale/garbage signature → `401` (timestamp guard); well-formed signature signed against a different body (tamper check) → `401 "Invalid webhook signature"`; fresh timestamp + fabricated digest → `401 "Invalid webhook signature"` (isolates the HMAC check specifically); a real signature generated via the actual `retell` SDK's own `sign()` function, matching the real body, against the seeded `practice-fixture-admin-001` → `200` with real data.
     - **Known, expected side effect**: 26 existing tests across `test_multi_tenant_isolation.py` (5) and `test_retell_hard_suite.py` (21) now fail, because they call these endpoints without sending a signature — this is the correct, intended consequence of enabling a check that was previously bypassed, not a bug in the fix. These need a follow-up (send a real signature via the same `sign()` pattern used to verify this fix) — not done yet, tracked below in Pending.
     - **Explicitly NOT deployed** — holding for a coordinated live-call verification against the real Retell integration before shipping, since a wrong `RETELL_API_KEY` value (unverified against Retell's actual live signing key — only inferred as likely correct from adjacent evidence, not directly confirmed) would silently break the live voice agent's every function call, a different and arguably worse failure mode than the current unauthenticated-but-working state.
     - **Investigated 2026-08-07: what safe verification options actually exist before deploying.** Findings:
       - **No sandbox/staging agent or second Retell account exists.** Checked `.env`, `.env.example`, and `service.yaml` — only `RETELL_API_KEY` and `RETELL_WEBHOOK_SECRET` are configured, no second key or environment-scoped config anywhere in the repo. Retell's own "environment tags" feature (researched via their docs) is a prompt/config-version switcher on the *same* agent and credentials, not an isolated sandbox with separate signing keys — doesn't help here.
       - **`backend/tests/retell_sim/` (previously cited as "verified 6/6 HTTP 200") does NOT exercise this fix.** Read `signing.py` and `scenario_runner.py` in full: `post_function()`, the helper used for all 7 function-call endpoints, never sends a signature at all — only `post_webhook()` (a different endpoint, `/api/retell/webhook`) and `post_summary()` (a third, separate shared-secret mechanism) sign anything. Traced via `git log` to commit `0c351e38` (2026-07-12) — its "6/6 HTTP 200" was passing *because* the auth bypass this fix closes already existed; it's not evidence the harness, or the old code, was secure, and it can't validate the new signature check as currently written.
       - **Retell's dashboard tools ("Test LLM" / LLM Playground, and a separate "Simulation and Batch Testing" feature) exist and are documented as not affecting live phone/production operations**, but Retell's public docs do not explicitly state whether an unmocked function call in these tools sends a genuinely-signed HTTP request identical to a real call, or whether it's fully mocked — this is inferred (not confirmed) from mock-related doc language implying "unmocked = the real action runs." Not confirmed either way despite direct doc searches.
       - **Structural constraint that limits every option below**: Retell's custom-function calls always go to the URL configured on the live agent — there is no documented way to point a Retell-side test at an arbitrary/local URL, so *any* verification using Retell's own signing infrastructure requires this fix to be deployed first. Local self-signing (already done) is the only zero-deploy option, and it can't confirm the real `RETELL_API_KEY` matches Retell's actual live signing key.
       - **Ranked options for the eventual deploy+verify step** (safest first, not yet acted on): (1) local self-signed test — done, proves logic only; (2) deploy as a Cloud Run `--no-traffic` revision, then test via Retell's dashboard *if* it supports pointing a single test at a non-default URL — unconfirmed this capability exists; (3) deploy normally, immediately run one **read-only** function (`list-providers`/`check-provider-availability`, not booking/cancelling) unmocked via Retell's Test LLM chat panel (not a phone call) before any real call can land; (4) one real, deliberate benign phone call by Darnell right after deploy; (5) deploy and passively wait for real traffic — not recommended, listed only to rule out.
       - **Monitoring gap flagged**: the existing Cloud Run alert policy fires on 5xx ratio; a `RETELL_API_KEY` mismatch would produce 401s, which that alert would not catch. Recommend a manual log check for `/api/retell/*` 401s immediately after whichever deploy/verify option is chosen.
  2. **Real MongoDB Atlas credentials hardcoded in plaintext, committed to git**: `backend/scripts/create_admin.py`, `init_db.py`, `add_booking_key_index.py` all contain the live DB username/password. Needs rotating in Atlas and removing from these files.
  - Moderate: `POST /api/llm/chat/completions` (the real LLM proxy, real cost) has no auth — confirmed live reachable without credentials. Cost-abuse risk, not a PHI risk.
...
efd2c0d0 docs: update HANDOFF.md
