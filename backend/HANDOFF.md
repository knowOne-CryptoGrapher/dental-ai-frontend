...
- Deploy the self-serve signup lockdown (`SELF_SERVE_TIERS_ENABLED`, committed as `a4f7589a`, see Done ✅) — batches with the above.
- **Task B2 — frontend "Coming Soon" UI for locked tiers, still pending, not started.** B1 (this session) only closed the backend gate; `PricingPage.jsx`, `LandingPage.jsx`, and `OnboardingWizard.js` still show Professional/Enterprise/Elite as normal, fully-clickable options — anyone clicking through today gets a 400 with no explanation in the UI. B2 needs to add "Coming Soon" badging/disabled states to those three surfaces once scoped.
