...
- **Full production security/leakage/load audit run 2026-08-07 against the live deploy — 2 critical findings + 1 moderate. Finding #1 is FIXED, DEPLOYED, and LIVE-VERIFIED as of 2026-08-08. Finding #2 and the moderate finding are still OPEN.** Full findings reported in-conversation, not duplicated in full here:
  ...
  2. **Real MongoDB Atlas credentials hardcoded in plaintext, committed to git**: `backend/scripts/create_admin.py`, `init_db.py`, `add_booking_key_index.py` all contained the live DB username/password.
     ...
     - **Separate, unrelated finding, flagged for future cleanup, not fixed**: a Cloud Build GitHub trigger (`rmgpgab-dental-ai-backend-v2-us-west1-knowOne-CryptoGrapher-xbl`, ID `213177b7-6cfc-40f1-b648-b2a76b9d4225`) is connected to this repo and fires automatically on every push to `unlocked-main`, but **every single build it has ever produced has failed** (confirmed back through 2026-07-15). Cross-checked against `gcloud builds list`: every build with this trigger ID shows `FAILURE`; every build with no trigger ID (the manual `gcloud builds submit` path `deploy_backend.ps1` actually uses) shows `SUCCESS` — including the build that produced today's live Retell-fix revision. Confirmed genuinely disconnected from real deploys, not currently blocking anything, but worth fixing or removing later.
  - **Moderate: `POST /api/llm/chat/completions` (the real LLM proxy, real cost) had no auth — confirmed live reachable without credentials. Cost-abuse risk, not a PHI risk.**
    - **STATUS: FIXED (2026-08-08), NOT YET DEPLOYED.** Committed as `6e74e8e2` (`fix(llm): require practice-scoped auth on /chat/completions, remove stale Retell-caller docstring`), pushed to `origin/unlocked-main`. Full test suite run locally, clean.
