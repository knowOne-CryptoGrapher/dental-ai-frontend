...
- **Full production security/leakage/load audit run 2026-08-07 against the live deploy — 2 critical findings, not yet fixed (audit-only, per instruction).** Full findings reported in-conversation, not duplicated in full here:
  1. **`retell_api_router.py`'s 7 patient/appointment/provider endpoints (lookup-patient, book-appointment, cancel-appointment, register-patient, etc.) have no real authentication in production** — they read an `x_retell_signature` header and API key but discard both (`_ = (retell_api_key, x_retell_signature, body_bytes)`); a working `verify_retell_signature()` HMAC function exists in the same file but is never called. Confirmed live: `list-providers` and `lookup-patient` both return real `200` data for a guessed `practice_id` with zero credentials. Anyone with a `practice_id` (a UUID, not a secret) can read/write patient and appointment data for any practice. Fix is small — wire up the existing verification function, matching the pattern already correct in `retell_webhook_router_v2.py` and `retell_context_router.py`.
