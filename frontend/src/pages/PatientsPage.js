...
export default function PatientsPage({ useMock = false }) {
  ...
  const openEdit = (patient) => {
    ...
    setForm({
      ...
      preferred_contact: patient.preferred_contact || 'phone',
      ...
      {loading ? (
        ...
        <div className="grid gap-3">
          ...
          {filtered.map((patient) => (
            <Card
              key={patient.id}
              className="border-gray-200/80 hover:shadow-sm transition-shadow cursor-pointer"
          ...
          {filtered.length === 0 && (
            <p className="text-sm text-gray-500 text-center py-12">No patients found</p>
          )}
        </div>
      )}
      ...
      <Dialog open={dialogOpen} onOpenChange={closeDialog}>
        <DialogContent className="max-w-md max-h-[85vh] overflow-y-auto">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {error && (
              <div className="p-2 rounded bg-red-50 border border-red-200 text-xs text-red-600 flex items-center gap-1">
              ...
              <div className="space-y-3 mt-2">
                <div className="grid grid-cols-2 gap-3 text-sm">
                  ...
                  <div>
                    ...
                    <p className="font-medium">{detailPatient.date_of_birth || '—'}</p>
                  </div>
                  <div>
                    <p className="text-xs text-gray-500">Preferred Contact</p>
                    <p className="font-medium capitalize">
                      {detailPatient.preferred_contact}
                    </p>
                  </div>
                </div>
                ...
                {detailPatient.emergency_contact_name && (
                  <div className="p-3 rounded-lg bg-red-50/50 border border-red-100">
                    ...
                    <p className="text-sm">
                      {detailPatient.emergency_contact_name} (
                      {detailPatient.emergency_contact_relationship}) —{' '}
                      {detailPatient.emergency_contact_phone}
                    </p>
                  </div>
                )}
