    ...
    try {
      if (useMock) {
        setAppointments(prev =>
          prev.map(a => a.id === appointmentId ? { ...a, status: 'scheduled' } : a)
        );
      } else {
        ...
        <div className="space-y-3">
          {filtered.map((apt) => {
            ...
            return (
              <Card
                key={apt.id}
                className={`border-gray-200/80 transition-all hover:shadow-sm ${
                  apt.status === 'pending_verification' ? 'border-l-4 border-l-amber-400' : ''
                } ${apt.is_emergency ? 'ring-1 ring-red-200' : ''}`}
              >
                <CardContent className="p-4">
                  ...
                  {apt.notes && (
                    <p className="text-xs text-gray-500 mt-2 ml-14">{apt.notes}</p>
                  )}
                </CardContent>
              </Card>
            );
            ...
            <div className="space-y-3">
              <div className="p-3 bg-gray-50 rounded-lg space-y-2 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">{verifyDialog.patient_name}</span>
                </div>

                <div className="flex justify-between">
                  <span className="text-gray-500">Date</span>
                  <span className="font-medium">{verifyDialog.appointment_date}</span>
                </div>

                <div className="flex justify-between">
                  <span className="text-gray-500">Time</span>
                  <span className="font-medium">{verifyDialog.appointment_time}</span>
                </div>
              ...
              <DialogFooter>
                ...
                <Button
                  ...
                  onClick={() => handleVerify(verifyDialog.id)}
                  disabled={verifying}
                >
                  {verifying && (
