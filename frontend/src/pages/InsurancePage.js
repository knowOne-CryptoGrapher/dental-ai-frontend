...
export default function InsurancePage() {
  ...
  const [claimForm, setClaimForm] = useState({
    ...
    carrier: '',
    policy_number: '',
    procedures: [{ code: '', description: '', fee: '' }]
  });
  ...
  const submitClaim = async () => {
    setSubmitting(true);
    try {
      const procs = claimForm.procedures
        .filter(p => p.code)
        .map(p => ({ ...p, fee: parseFloat(p.fee) || 0 }));

      await api.post('/insurance/submit-claim', {
        ...claimForm,
        procedures: procs
      });
  ...
  return (
    <div className="space-y-6 max-w-7xl">
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-500">
                          {claim.carrier_name || claim.carrier} ·{' '}
                          {claim.procedures?.length || 0} procedures · $
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs text-gray-400">
                        {claim.submission_method?.toUpperCase()}
                      </span>
                    </div>
                  </div>
                );
      ...
      <Card className="border-gray-200/80">
        <CardHeader className="pb-3">
          <CardTitle className="text-base">Supported Carriers</CardTitle>
        </CardHeader>
          ...
          {eligResult ? (
            <div className="space-y-3">
              <div className="p-3 bg-emerald-50 border border-emerald-200 rounded-lg">
                ...
                <p className="text-xs text-emerald-600 mt-1">
                  {eligResult.carrier_name} · {eligResult.policy_number}
                </p>
              </div>

              {eligResult.coverage &&
                Object.entries(eligResult.coverage).map(([k, v]) => (
                  <div
                    key={k}
                  ...
                  >
                    ...
                    <span className="font-medium">
                      {v.percentage}% · ${v.remaining || v.annual_max} remaining
                    </span>
                  </div>
                ))}

              <p className="text-[10px] text-amber-600 bg-amber-50 p-2 rounded">
                MOCKED RESPONSE — Connect ITRANS/CDANet for live data
              </p>
            ...
            <div className="space-y-3">
              <div className="space-y-1.5">
                <Label>Carrier</Label>
                <select
                  value={eligForm.carrier}
                  onChange={e =>
                    setEligForm({ ...eligForm, carrier: e.target.value })
                  }
              ...
              <div className="space-y-1.5">
                ...
                <Input
                  value={eligForm.policy_number}
                  onChange={e =>
                    setEligForm({ ...eligForm, policy_number: e.target.value })
                  }
                />
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <Input
                  value={eligForm.patient_name}
                  onChange={e =>
                    setEligForm({ ...eligForm, patient_name: e.target.value })
                  }
                />
              </div>

              <DialogFooter>
                <Button variant="outline" onClick={() => setEligDialog(false)}>
                  Cancel
                </Button>
          ...
          <div className="space-y-3">
            <div className="space-y-1.5">
              ...
              <select
                ...
                onChange={e =>
                  setClaimForm({ ...claimForm, carrier: e.target.value })
                }
                className="w-full h-10 rounded-md border border-gray-200 bg-white px-3 text-sm"
              >
                <option value="">Select carrier</option>
            ...
            <div className="grid grid-cols-2 gap-2">
              <div className="space-y-1.5">
                <Label>Policy #</Label>
                <Input
                  data-testid="claim-policy-input"
                  value={claimForm.policy_number}
                  onChange={e =>
                    setClaimForm({ ...claimForm, policy_number: e.target.value })
                  }
                />
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <select
                  ...
                  onChange={e =>
                    setClaimForm({ ...claimForm, patient_id: e.target.value })
                  }
                  className="w-full h-10 rounded-md border border-gray-200 bg-white px-3 text-sm"
                >
              ...
              {claimForm.procedures.map((p, i) => (
                <div key={i} className="grid grid-cols-3 gap-1.5 mt-1.5">
                  <Input
                    ...
                    onChange={e => {
                      const procs = [...claimForm.procedures];
                      procs[i].code = e.target.value;
                      setClaimForm({ ...claimForm, procedures: procs });
                    }}
                  />
                  ...
                  <Input
                    ...
                    onChange={e => {
                      const procs = [...claimForm.procedures];
                      procs[i].description = e.target.value;
                      setClaimForm({ ...claimForm, procedures: procs });
                    }}
                  />
            ...
            <p className="text-[10px] text-amber-600 bg-amber-50 p-2 rounded">
              MOCKED — Claims are simulated. Connect ITRANS/CDANet for live submission.
            </p>
