    ...
    <div className="space-y-6 max-w-7xl">
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          ...
          {filtered.length === 0 && (
            <p className="text-sm text-gray-500 text-center py-12">No call logs found</p>
          )}
        </div>
      )}
                ...
                <DialogTitle className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs font-normal text-gray-500">
                      {selectedCall.patient_phone} · {formatDuration(selectedCall.duration)}
                    </p>
                  </div>
                </DialogTitle>
              </DialogHeader>
              ...
              <div className="space-y-4 mt-2">
                    ...
                    <div className="grid grid-cols-2 gap-2 text-sm">
                      ...
                      {selectedCall.call_summary.notes && (
                        <div className="col-span-2">
                          ...
                          <p className="text-gray-800">{selectedCall.call_summary.notes}</p>
                        </div>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="bg-gray-50 rounded-lg p-4 max-h-64 overflow-y-auto">
                    {selectedCall.transcript?.split('\n').map((line, i) => {
                      ...
                      return (
                        <div
                          key={i}
                          className={`mb-2 ${isAI ? 'pl-0' : isCaller ? 'pl-4' : ''}`}
                        >
                          {isAI && (
                ...
                <div className="grid grid-cols-2 gap-3 text-sm border-t border-gray-100 pt-3">
                  ...
                  <div>
                    ...
                    <p className="font-medium uppercase">{selectedCall.handled_by}</p>
                  </div>

                  <div>
                    <span className="text-xs text-gray-500">Action Taken</span>
                    <p className="font-medium">{selectedCall.action_taken || '—'}</p>
                  </div>

                  <div>
                    <span className="text-xs text-gray-500">Time</span>
                    <p className="font-medium">{formatTime(selectedCall.timestamp)}</p>
                  </div>
                </div>
              </div>
            </>
